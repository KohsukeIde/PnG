...
def test_matching_evaluator_extract_matches():
    ...
    solver = OptimalTransportSolver(
        ...
        gaussians2,
        epsilon=1.0,
        lambda_color=0.0,  # Changed epsilon to 1.0
    )
...
def test_matching_evaluator_evaluate_matches_complete_match():
    ...
    expected_distance = (
        0.0 + np.linalg.norm(np.array([1, 1]) - np.array([1, 1]))
    ) / 2  # (0 + 0) / 2 = 0.0
    expected_color_diff = (
        0.0 + np.linalg.norm(np.array([0, 1, 0]) - np.array([0, 1, 0]))
    ) / 2  # (0 + 0) / 2 = 0.0
...
def test_matching_evaluator_visualize_matches():
    ...
    transport_matrix = np.array([[1.0, 0.0], [0.0, 1.0]])
