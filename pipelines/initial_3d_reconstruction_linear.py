...
def save_ellipsoids_as_ply(all_vertices, all_faces, filename):
    ...
    with open(filename, 'w') as f:
        ...
        # We have x,y,z plus color+alpha => total 7 properties
...
def main():
    ...
    if image1_id is None or image2_id is None:
        print(f"Error: Could not find either {image1_name} or {image2_name} in COLMAP data.")
        sys.exit(1)
    ...
    # 3) Setup OptimalTransportSolver (with alpha in cost)
    ...
    # 4) Homography initialization + optimize
    ...
    # 9) For each 3D Gaussian, build ellipsoid + store color+alpha => PLY
    all_vertices = []
    all_faces = []
    n_theta, n_phi = 12, 12
    ...
    for i in range(points_3d.shape[0]):
        ...
        for vert in raw_vertices:
            # combine => (x,y,z, r, g, b, a)
            combined = np.concatenate([vert, c3, [a3]])
            extended_vertices.append(combined)

        all_vertices.append(extended_vertices)
        all_faces.append(faces)

    # 10) Save colored ellipsoids
    ply_ellipsoids_path = os.path.join('results', '3d_gaussians_ellipsoids.ply')
    os.makedirs('results', exist_ok=True)
    save_ellipsoids_as_ply(all_vertices, all_faces, ply_ellipsoids_path)
    print(f"\n3D Gaussian ellipsoids saved to {ply_ellipsoids_path}")

    # 11) Save everything to pickle
    results = {
