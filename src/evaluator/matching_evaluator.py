...
class MatchingEvaluator:
    """A class for evaluating and visualizing matches between two sets of 2D Gaussians.

    This class takes two TwoDGaussians objects and a transport matrix, and provides
    methods to extract matches, evaluate their quality, and visualize the results.
    """
    ...
    def __init__(
        ...
        """Initialize the MatchingEvaluator.

        Args:
            gaussians1 (TwoDGaussians): First set of 2D Gaussians.
            gaussians2 (TwoDGaussians): Second set of 2D Gaussians.
            transport_matrix (np.ndarray): Transport matrix between the two sets of Gaussians.
        """
    ...
    def extract_matches(self) -> list:
        ...
        cost_matrix = np.full(
            (size, size), fill_value=np.max(self.transport_matrix) * 2
        )
        ...
        for i, j in zip(row_ind, col_ind):
            if (
                i < num_rows
                and j < num_cols
                and self.transport_matrix[i, j] > threshold
            ):
    ...
    def evaluate_matches(self) -> Dict[str, float]:
        ...
        distances = [
            np.linalg.norm(self.gaussians1.means[i] - self.gaussians2.means[j])
        ...
        color_diffs = [
            np.linalg.norm(self.gaussians1.rgb[i] - self.gaussians2.rgb[j])
            for i, j in self.matches
        ]
        avg_color_diff = np.mean(color_diffs)

        # Calculate matching rate (average of the number of Gaussians in both distributions)
        matching_rate = len(self.matches) / (
            (self.gaussians1.k + self.gaussians2.k) / 2
        )
    ...
    def visualize_matches(self, output_path: str, top_k: int = 100) -> None:
        ...
        ax.scatter(
            self.gaussians1.means[:, 0],
            self.gaussians1.means[:, 1],
            c=self.gaussians1.rgb,
            marker="o",
            label="Image1 Gaussians",
        )
        ...
        ax.scatter(
            shifted_means2[:, 0],
            shifted_means2[:, 1],
            c=self.gaussians2.rgb,
            marker="x",
            label="Image2 Gaussians",
        )
        ...
        # Visualize matches
        for i, j, _ in matches_with_transport:
            point1 = self.gaussians1.means[i]
            point2 = self.gaussians2.means[j] + np.array([offset_x, 0])
            ax.plot(
                [point1[0], point2[0]], [point1[1], point2[1]], "k--", linewidth=0.5
            )

        ax.legend()
        ax.set_title(
            f"Gaussian Matching Visualization (Top {top_k} Matches)"
            if top_k
            else "Gaussian Matching Visualization"
        )
        ax.set_xlabel("X-axis")
        ax.set_ylabel("Y-axis")
        plt.tight_layout()
